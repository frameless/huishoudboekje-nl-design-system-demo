...
public class UpdateIsReconciledConsumer(ITransactionService transactionServices,  IConfiguration config)
  : IConsumer<UpdateIsReconciledMessage>
{
  public async Task Consume(ConsumeContext<UpdateIsReconciledMessage> context)
  {
    IList<string>? ids = context.Message.Ids;
    if (ids != null)
    {
      await transactionServices.UpdateIsReconciled(ids, context.Message.IsReconciled);
    }

    UpdateIsReconciledResponse result = new()
    {
      updated = true
    };
