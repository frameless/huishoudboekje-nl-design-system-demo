...
ENV HHB_DATABASE_URL "Host=db;Database=fileservice;Port=5432;Username=postgres;Password=postgres"
WORKDIR /app
COPY . .
RUN dotnet restore ./FileServices.Application/FileServices.Application.csproj
ENTRYPOINT dotnet run --urls=http://+:8000 --project ./FileServices.Application/FileServices.Application.csproj --launch-profile dev-docker --no-restore
