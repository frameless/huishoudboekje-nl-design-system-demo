...
def place_files_in_operations(operations, files_map, files):
    ...
    for path, key in path_to_key_iter:
        ...
        if file_obj:
            output = _add_file_to_operations(output, file_obj, path)
    return output


def _add_file_to_operations(operations, file_obj, path):
    ...
    if isinstance(operations, dict):
        key = path[0]
        sub_dict = _add_file_to_operations(operations[key], file_obj, path[1:])
        return _new_merged_dict(operations, {key: sub_dict})
    if isinstance(operations, list):
        index = int(path[0])
        sub_item = _add_file_to_operations(
            operations[index],
            file_obj,
            path[1:],
        )
        return _new_list_with_replaced_item(operations, index, sub_item)
    raise TypeError('Operations must be a dict or a list of dicts')


def _new_merged_dict(*dicts):
...
def _new_list_with_replaced_item(input_list, index, new_value):
