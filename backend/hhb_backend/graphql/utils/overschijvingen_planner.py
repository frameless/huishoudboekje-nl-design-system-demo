...
def planned_overschrijvingen(afspraak, start_datum=None, eind_datum=None):
    ...
    if afspraak["aantal_betalingen"] > 0:
        payments += get_normal_afspraak_overschijvingen(afspraak, payment_date, time_delta, start_datum, eind_datum)
    else:
        payments += get_doorlopende_afspraak_overschijvingen(afspraak, payment_date, time_delta, start_datum, eind_datum)

    return payments

def get_normal_afspraak_overschijvingen(afspraak, payment_date, time_delta, start_datum=None, eind_datum=None):
    payments = []
    ...
    for payment_id in range(afspraak["aantal_betalingen"]):
        ...
        if not start_datum or payment_date >= start_datum:
            payments.append(make_overschijving_dict(payment_amount, payment_date))
        payment_date = payment_date + time_delta
        if eind_datum and payment_date > eind_datum:
            break
    return payments

def get_doorlopende_afspraak_overschijvingen(afspraak, payment_date, time_delta, start_datum=None, eind_datum=None):
    payments = []
    payment_id = 0
    while payment_date <= eind_datum:
        if not start_datum or payment_date >= start_datum:
            payments.append(make_overschijving_dict(afspraak["bedrag"], payment_date))
        payment_date = payment_date + time_delta
        payment_id += 1
    return payments

def make_overschijving_dict(bedrag, date):
    return {
        "id": None,
        "bedrag": bedrag,
        "datum": date,
