...
def create_export_string(verwerkingdatum, overschrijvingen, afspraken, tegen_rekeningen, config_values):
    config = {
    ...
    for overschrijving in overschrijvingen:
        ...
        tegen_rekening = next(filter(
            lambda x: x['id'] == afspraak['tegen_rekening_id'], tegen_rekeningen), None)
        payment = {
            "name": tegen_rekening["rekeninghouder"],
            "IBAN": tegen_rekening["iban"],
            # "BIC": "BANKNL2A", # TODO nodig??
            "amount": overschrijving['bedrag'],
            "execution_date": to_date(overschrijving['datum']) if verwerkingdatum is None else verwerkingdatum,
    ...
    sepaxmlstring = sepaxmlstring.replace(
        "<BtchBookg>true</BtchBookg>", "<BtchBookg>false</BtchBookg>")
