...
mock_rubriek_post_success_repsonse = {"data":
    {"id": 11, "grootboekrekening_id": "m12", "naam": "test rubriek"},
}
...
def setup_services(mock):
    ...
    rubrieken_adapter = mock.post(f"{settings.HHB_SERVICES_URL}/rubrieken/",
...
def test_create_rubriek_success(client):
    with requests_mock.Mocker() as mock:
        ...
        assert not adapters["grootboekrekeningen2"].called
        assert adapters["grootboekrekeningen3"].called_once

def test_create_rubriek_unknown_grootboek(client):
    with requests_mock.Mocker() as mock:
        adapters = setup_services(mock)
        ...
        assert response.json['errors'][0]['message'] == 'Grootboekrekening id [WRONG] not found.'
