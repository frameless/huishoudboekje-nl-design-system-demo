...
def test_saldo_controller_no_burger_ids():
    ...
    sut = SaldoController(mock_hhbservice_repository,mock_banktransactionservice_repository)
    ...
    mock_banktransactionservice_repository.get_saldo.assert_called_with(date, transactions=None)
...
def test_saldo_controller_burger_ids():
    ...
    assert result == {"saldo": saldo}
    assert mock_banktransactionservice_repository.get_saldo.called
    mock_banktransactionservice_repository.get_saldo.assert_called_with(date, transactions=transaction_ids_list)
    assert mock_hhbservice_repository.get_transaction_ids_burgers.called
    mock_hhbservice_repository.get_transaction_ids_burgers.assert_called_with(burger_ids)
