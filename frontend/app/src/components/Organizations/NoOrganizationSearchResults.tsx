...
const NoOrganizationSearchResults: React.FC<BoxProps & { onSearchReset?: Function }> = ({onSearchReset}) => {
	...
	return (
		<Stack justifyContent={"center"} alignItems={"center"} bg={"white"} p={20} spacing={10} borderRadius={5}>
			...
			<Text fontSize={"sm"}>{t("messages.organizations.noSearchResults")}</Text>
			{onSearchReset && <Button size="sm" variantColor="primary" onClick={() => onSearchReset()}>{t("actions.clearSearch")}</Button>}
		</Stack>
	);
};

export default NoOrganizationSearchResults;