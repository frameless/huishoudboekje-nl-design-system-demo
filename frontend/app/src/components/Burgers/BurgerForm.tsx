...
const BurgerForm: React.FC<BurgerFormProps> = ({
  burger,
  onSubmit,
  isLoading,
  isBsnValid = true,
  ...
  const [form, { updateForm, toggleSubmitted, isFieldValid }] = useForm<
  ...
  >({
    ...
    initialValue: {
      ...
      geboortedatum: geboortedatum
        ? d(geboortedatum, "YYYY-MM-DD").format("L")
        : null,
      email,
      huisnummer,
      postcode,
  ...
  const onSubmitForm = (e) => {
    ...
    try {
      ...
      onSubmit({
        ...
        bsn: Number(data.bsn),
        geboortedatum: d(data.geboortedatum, "L").isValid()
          ? d(data.geboortedatum, "L").format("YYYY-MM-DD")
          : null,
      });
    } catch (err) {
      toast.closeAll();
  ...
  return (
    <Box as={"form"} onSubmit={onSubmitForm}>
      <SectionContainer>
        <Section
          title={t("forms.burgers.sections.personal.title")}
          helperText={t("forms.burgers.sections.personal.helperText")}
        >
          <Stack>
            <Stack spacing={2} direction={["column", "row"]}>
              <FormControl
                id={"bsn"}
                isInvalid={!isFieldValid("bsn") || !isBsnValid}
                isRequired={true}
              >
                <FormFieldTextbox
                  label={t("forms.burgers.fields.bsn")}
                  autoComplete="no"
                  aria-autocomplete="none"
                  onChange={(e: any) => updateForm("bsn", e.target.value)}
                  value={form.bsn || ""}
                  errorMessage={t("messages.burgers.invalidBsn")}
                />
              </FormControl>
            </Stack>
            <Stack spacing={2} direction={["column", "row"]}>
              <FormControl
                id={"voorletters"}
                isInvalid={!isFieldValid("voorletters")}
                isRequired={true}
              >
                <Stack spacing={1} flex={1}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidVoorletters")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
              <FormControl
                id={"voornamen"}
                isInvalid={!isFieldValid("voornamen")}
                isRequired={true}
              >
                <Stack spacing={1} flex={3}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidVoornamen")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
              <FormControl
                id={"achternaam"}
                isInvalid={!isFieldValid("achternaam")}
                isRequired={true}
              >
                <Stack spacing={1} flex={3}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidAchternaam")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
            </Stack>
            <FormControl
              id={"geboortedatum"}
              isInvalid={!isFieldValid("geboortedatum")}
            >
              <Stack spacing={1}>
                <FormLabel>{t("forms.burgers.fields.geboortedatum")}</FormLabel>
                <DatePicker
                  selected={
                    d(form.geboortedatum, "L").isValid()
                      ? d(form.geboortedatum, "L").toDate()
                      : null
                  }
                  autoComplete="no"
                  aria-autocomplete="none"
        ...
        <Section
          title={t("forms.burgers.sections.contact.title")}
          helperText={t("forms.burgers.sections.contact.helperText")}
        >
          <Stack>
            <Stack spacing={2} direction={["column", "row"]}>
              <FormControl
                id={"straatnaam"}
                isInvalid={!isFieldValid("straatnaam")}
                isRequired={true}
              >
                <Stack spacing={1} flex={2}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidStraatnaam")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
              <FormControl
                id={"huisnummer"}
                isInvalid={!isFieldValid("huisnummer")}
                isRequired={true}
              >
                <Stack spacing={1} flex={1}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidHuisnummer")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
            </Stack>
            <Stack spacing={2} direction={["column", "row"]}>
              <FormControl
                id={"postcode"}
                isInvalid={!isFieldValid("postcode")}
                isRequired={true}
              >
                <Stack spacing={1} flex={1}>
                  <FormLabel>{t("forms.burgers.fields.postcode")}</FormLabel>
                  <Tooltip
                    label={t("forms.burgers.tooltips.postcode")}
                    aria-label={t("forms.burgers.fields.postcode")}
                  ...
                  <FormErrorMessage>
                    {t("messages.burgers.invalidPostcode")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
              <FormControl
                id={"plaatsnaam"}
                isInvalid={!isFieldValid("plaatsnaam")}
                isRequired={true}
              >
                <Stack spacing={1} flex={2}>
                  ...
                  />
                  <FormErrorMessage>
                    {t("messages.burgers.invalidPlaatsnaam")}
                  </FormErrorMessage>
                </Stack>
              </FormControl>
            </Stack>
            <FormControl
              id={"telefoonnummer"}
              isInvalid={!isFieldValid("telefoonnummer")}
            >
              <Stack spacing={1}>
                <FormLabel>
                  {t("forms.burgers.fields.telefoonnummer")}
                </FormLabel>
                <Tooltip
                  label={t("forms.burgers.tooltips.telefoonnummer")}
                  aria-label={t("forms.burgers.tooltips.telefoonnummer")}
                  placement={isMobile ? "top" : "left"}
                >
                  <Input
                    onChange={(e) =>
                      updateForm("telefoonnummer", e.target.value || null)
                    }
                ...
                <FormErrorMessage>
                  {t("messages.burgers.invalidTelefoonnummer")}
                </FormErrorMessage>
              </Stack>
            </FormControl>
            <FormControl id={"mail"} isInvalid={!isFieldValid("email")}>
              <Stack spacing={1}>
                ...
                />
                <FormErrorMessage>
                  {t("messages.burgers.invalidEmail")}
                </FormErrorMessage>
              </Stack>
            </FormControl>
          </Stack>
