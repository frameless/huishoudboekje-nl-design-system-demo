...
@pytest.mark.parametrize("gebruiker_data,expected", [
    ({"telefoonnummer": "0612345678", "geboorterdatum": "2020-01-01"},
        "'email' is a required property"),
    ({"email": "a@b.c", "geboorterdatum": "2020-01-01"},
        "'telefoonnummer' is a required property"),
    ({"email": "a@b.c", "telefoonnummer": "0612345678"},
        "'geboortedatum' is a required property")
])
def test_gebruikers_post_input_json_validation(app, gebruiker_data, expected):
    """ Test JSON input validation of new gebruiker """
    response = app.test_client().post('/gebruikers',
        data=json.dumps(gebruiker_data), content_type='application/json')
    assert response.status_code == 400
    assert response.json["errors"][0] == expected
